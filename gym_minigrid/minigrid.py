...
class ColorDoor(Door):
    def __init__(self, color, is_open=False):
        super().__init__(color, is_open, False)
...
class Agent(WorldObj):
    def __init__(self, index=0, view_size=7, agent_type=None):
...
class MiniGridEnv(gym.Env):
    ...
    def place_goal(
        ...
        goal.pos = None
        pos = self.place_obj(goal, top, size, max_tries=max_tries)
        goal.pos = pos
    ...
    def step(self, actions):
        ...
        for action, agent in zip(actions, self.agents):
            ...
            # Toggle/activate an object
                ...
                if fwd_cell:
                    success = fwd_cell.toggle(self, fwd_pos, agent)
                    if fwd_cell.type == 'door' and success:
                        self.door_num -= 1
                        if self.door_num == 0:
                            agent.done = True
                            reward = self._reward()
            ...
            agent_done = agent_done and agent.done
    ...
    def gen_obs(self):
        ...
        obs = []
        for grid, vis_mask, agent in zip(grids, vis_masks, self.agents):
            ...
            for agent_id, a in enumerate(self.agents):
                ...
                for vis_j in range(0, self.agent_view_size):
                    for vis_i in range(0, self.agent_view_size):
                        ...
                        if not vis_masks[agent_id][vis_i, vis_j]:
                            continue
